...
class UsuarioPermissoesSeeder extends Seeder
{
    public function run()
    {
        ...
        // $this->db->table('usuario_permissoes')->truncate(); // Descomente para limpar antes de inserir
        if (!empty($data)) {
            $this->db->table('usuario_permissoes')->insertBatch($data);
        } else {
            // Se o array de dados estiver vazio, você pode optar por não fazer nada
            // ou logar uma mensagem.
            // $this->db->table('usuario_permissoes')->where('id IS NOT NULL')->delete(); // Exemplo: limpar a tabela se o seed for vazio
        }
    }
}