    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-sm-flex align-items-center">
            ...
            <div class="col-md-auto">
                <?php if (service('Carol')->pode('CARGOS.NOVO')) : ?>
                <a href="<?= base_url('cargos/new') ?>" class="btn btn-primary shadow-sm">
                        ...
                        <?php if (!empty($registros)): ?>
                            <?php foreach ($registros as $registro): ?>
                                <tr>
                                    ...
                                    <td>
                                      <?php if (service('Carol')->pode('CARGOS.VER')) : ?>
                                        <a href="<?= base_url('cargos/' . $registro->id) . '/show' ?>" class="btn btn-info btn-sm" title="Detalhes">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                      <?php endif; ?>

                                      <?php if (service('Carol')->pode('CARGOS.EDITAR')) : ?>
                                      <a href="<?= base_url('cargos/' . $registro->id . '/edit') ?>" class="btn btn-warning btn-sm" title="Editar">
                                          <i class="bi bi-pencil"></i>
                                      </a>
                                      <?php endif; ?>

                                      <?php if (service('Carol')->pode('CARGOS.EXCLUIR')) : ?>
                                      <form action="<?= base_url('cargos/' . $registro->id) ?>" method="POST" class="d-inline form-delete">
                                        <?= csrf_field() ?>
