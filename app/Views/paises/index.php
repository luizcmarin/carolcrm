    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-sm-flex align-items-center">
            ...
            <div class="col-md-auto">
                ...
                <a href="<?= base_url('paises/new') ?>" class="btn btn-primary shadow-sm">
                    ...
                    <tbody class="table-group-divider">
                        <?php if (!empty($registros)): ?>
                            <?php foreach ($registros as $registro): ?>
                                <tr>
                                    <td><?= $registro->iso2 ?></td>
                                    <td><?= $registro->nome_curto ?></td>
                                    <td><?= $registro->nome_longo ?></td>
                                    <td><?= $registro->iso3 ?></td>
                                    <td><?= $registro->codigo_numerico ?></td>
                                    <td><?= $registro->codigo_chamada ?></td>
                                    <td><?= $registro->cctld ?></td>
                                    <td><?= $registro->criado_em ?></td>
                                    <td><?= $registro->editado_em ?></td>
                                    <td>
                                      <?php if (service('Carol')->pode('PAISES.VER')) : ?>
                                        <a href="<?= base_url('paises/' . $registro->id) . '/show' ?>" class="btn btn-info btn-sm" title="Detalhes">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                      <?php endif; ?>

                                      <?php if (service('Carol')->pode('PAISES.EDITAR')) : ?>
                                      <a href="<?= base_url('paises/' . $registro->id . '/edit') ?>" class="btn btn-warning btn-sm" title="Editar">
                                          <i class="bi bi-pencil"></i>
                                      </a>
                                      <?php endif; ?>

                                      <?php if (service('Carol')->pode('PAISES.EXCLUIR')) : ?>
                                      <form action="<?= base_url('paises/' . $registro->id) ?>" method="POST" class="d-inline form-delete">
                                        <?= csrf_field() ?>
