...
<div class="container-fluid">
  ...
  <div class="card shadow mb-4">
    ...
    <div class="card-body">
      <form action="<?= base_url('cotacoes/' . $registros->id) ?>" method="post">
          <?= csrf_field() ?>
          <input type="hidden" name="_method" value="PUT">
                          ...
                          <select class="form-select choices-select" id="pais_id" name="pais_id" required>
                              ...
                              <?php
                              $selectedValue = old('pais_id', $cotacoes->pais_id ?? '');
                    ...
                    <div class="input-group">
                      <input type="number" class="form-control text-center" id="valor_cotacao" name="valor_cotacao"
                              placeholder="Valor Cotacao" value="<?= old('valor_cotacao', esc($registros->valor_cotacao ?? '')) ?>"
                              min="0" step="1" required>
                        <label for="valor_cotacao">Valor Cotacao</label>
                    ...
                    <button class="btn btn-sm btn-light copy-button-textarea" type="button" 
                            data-clipboard-target="#valor_cotacao" 
                            title="Copiar">
                        <i class="bi bi-clipboard"></i>
                    </button>
                    <span class="copy-feedback-message-external"></span>
                    </div>
                    <?php if (session('errors.valor_cotacao')) : ?>
                        <div class="invalid-feedback d-block">
                            <?= session('errors.valor_cotacao') ?>
                        </div>
                    <?php endif ?>
