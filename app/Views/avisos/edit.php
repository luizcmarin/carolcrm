...
<div class="container-fluid">
  ...
  <div class="card shadow mb-4">
    ...
    <div class="card-body">
      <form action="<?= base_url('avisos/' . $registros->id) ?>" method="post">
          <?= csrf_field() ?>
          <input type="hidden" name="_method" value="PUT">
                  ...
                  <input type="text" class="form-control" id="mensagem" name="mensagem" placeholder="Mensagem" value="<?= old('mensagem', esc($registros->mensagem ?? '')) ?>" >
                  <label for="mensagem">Mensagem</label>
                    <button class="btn btn-sm btn-light copy-button-textarea" type="button" 
                            data-clipboard-target="#mensagem" 
                            title="Copiar">
                        <i class="bi bi-clipboard"></i>
                    </button>
                    <span class="copy-feedback-message-external"></span>
                    ...
                    <input type="date" class="form-control datepicker-input" id="data_inicio" name="data_inicio" placeholder="Data Inicio" 
                        value="<?= old('data_inicio', esc($registros->data_inicio ?? '')) ?>" 
                        required>
                    <label for="data_inicio">Data Inicio</label>
                <button class="btn btn-sm btn-light copy-button-textarea" type="button" 
                        data-clipboard-target="#data_inicio" 
                        title="Copiar">
                    <i class="bi bi-clipboard"></i>
                </button>
                <span class="copy-feedback-message-external"></span>
                    <?php if (session('errors.data_inicio')) : ?>
                        <div class="invalid-feedback d-block">
                            <?= session('errors.data_inicio') ?>
                        </div>
                    <?php endif ?>
                </div>                <div class="form-floating mb-3 input-with-copy">
                    <input type="date" class="form-control datepicker-input" id="data_fim" name="data_fim" placeholder="Data Fim" 
                        value="<?= old('data_fim', esc($registros->data_fim ?? '')) ?>" 
                        required>
                    <label for="data_fim">Data Fim</label>
                <button class="btn btn-sm btn-light copy-button-textarea" type="button" 
                        data-clipboard-target="#data_fim" 
                        title="Copiar">
                    <i class="bi bi-clipboard"></i>
                </button>
                <span class="copy-feedback-message-external"></span>
                    <?php if (session('errors.data_fim')) : ?>
                        <div class="invalid-feedback d-block">
                            <?= session('errors.data_fim') ?>
                        </div>
                    <?php endif ?>
