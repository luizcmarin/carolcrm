...
class Cidades extends BaseController
{
    ...
    {
        $this->titulo = 'Tabela->Cidades';
        $this->model = new CidadesModel();
    ...
    { 
      if (!$this->Carol->pode('CIDADES.INDEX')) {
    ...
    public function create(): string|ResponseInterface
    {
      if (!$this->Carol->pode('CIDADES.NOVO')) {
        ...
        if ($this->model->save($registro)) {
            $this->logAtividadesModel->addLog('Cidades: novo registro [' . $registro->id . ' - ' . $registro->nome . ']');
            return redirect()->to('/cidades')->with('success', 'Registro criado com sucesso!');
        } else {
    ...
    {
      if (!$this->Carol->pode('CIDADES.EDITAR')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
        ...
        if (!$registros) {
            return redirect()->to('/cidades')->with('error', 'Registro não encontrado.');
        }
    ...
    public function update(int $id): string|ResponseInterface
    {
      if (!$this->Carol->pode('CIDADES.EDITAR')) {
        ...
        if ($this->model->save($registros)) {
            $this->logAtividadesModel->addLog('Cidades: registro atualizado [' . $id . '-' . $registro->nome . ']');
            return redirect()->to('/cidades')->with('success', 'Registro atualizado com sucesso!');
        } else {
            return redirect()->back()->withInput()->with('errors', $this->model->errors());
        }
    }
    ...
    {
      if (!$this->Carol->pode('CIDADES.VER')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
    ...
    {
      if (!$this->Carol->pode('CIDADES.EXCLUIR')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
      ...
      $nome = $this->model->getCampo('nome', ['id' => $id]);
        ...
        if ($this->model->delete($id)) {
            $this->logAtividadesModel->addLog('Cidades: registro excluído [' . $id . '-' . $nome . ']');
            return redirect()->to('/cidades')->with('success', 'Registro excluído com sucesso!');
        } else {
