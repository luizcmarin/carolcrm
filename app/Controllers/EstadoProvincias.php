...
use App\Models\EstadoProvinciasModel;
use App\Entities\EstadoProvincia;
use CodeIgniter\HTTP\ResponseInterface;

class EstadoProvincias extends BaseController
{
    protected $titulo; 
    ...
    {
        $this->titulo = 'Tabela->EstadoProvincias';
        $this->model = new EstadoProvinciasModel();
    ...
    { 
      if (!$this->Carol->pode('ESTADOPROVINCIAS.INDEX')) {
      ...
      return view('estadoprovincias/index', $data);
    }
    ...
    {
      if (!$this->Carol->pode('ESTADOPROVINCIAS.NOVO')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }

        $data = [
            'titulo' => 'Tabela->EstadoProvincias',
        ];

        // carrega dados relacionados
        // $data['tabela_options'] = $this->tabelaModel->getDropdown('id', 'nome', [], 'nome', 'ASC');

        return view('estadoprovincias/new', $data);
    }
    ...
    public function create(): string|ResponseInterface
    {
      if (!$this->Carol->pode('ESTADOPROVINCIAS.NOVO')) {
        ...
        $registro = new EstadoProvincia();
        $registro->fill($postData);

        if ($this->model->save($registro)) {
            $this->logAtividadesModel->addLog('EstadoProvincias: novo registro [' . $registro->id . ' - ' . $registro->nome . ']');
            return redirect()->to('/estadoprovincias')->with('success', 'Registro criado com sucesso!');
        } else {
    ...
    {
      if (!$this->Carol->pode('ESTADOPROVINCIAS.EDITAR')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
        ...
        if (!$registros) {
            return redirect()->to('/estadoprovincias')->with('error', 'Registro não encontrado.');
        }
        ...
        return view('estadoprovincias/edit', $data);
    }
    ...
    public function update(int $id): string|ResponseInterface
    {
      if (!$this->Carol->pode('ESTADOPROVINCIAS.EDITAR')) {
        ...
        if ($this->model->save($registros)) {
            $this->logAtividadesModel->addLog('EstadoProvincias: registro atualizado [' . $id . '-' . $registro->nome . ']');
            return redirect()->to('/estadoprovincias')->with('success', 'Registro atualizado com sucesso!');
        } else {
            return redirect()->back()->withInput()->with('errors', $this->model->errors());
        }
    }
    ...
    {
      if (!$this->Carol->pode('ESTADOPROVINCIAS.VER')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
        ...
        return view('estadoprovincias/show', $data);
    }
    ...
    {
      if (!$this->Carol->pode('ESTADOPROVINCIAS.EXCLUIR')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
      ...
      $nome = $this->model->getCampo('nome', ['id' => $id]);
        ...
        if ($this->model->delete($id)) {
            $this->logAtividadesModel->addLog('EstadoProvincias: registro excluído [' . $id . '-' . $nome . ']');
            return redirect()->to('/estadoprovincias')->with('success', 'Registro excluído com sucesso!');
        } else {
