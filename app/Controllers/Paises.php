...
class Paises extends BaseController
{
    ...
    { 
      if (!$this->Carol->pode('PAISES.INDEX')) {
    ...
    public function create(): string|ResponseInterface
    {
      if (!$this->Carol->pode('PAISES.NOVO')) {
        ...
        if ($this->model->save($registro)) {
            $this->logAtividadesModel->addLog('Paises: novo registro [' . $registro->id . ' - ' . $registro->nome . ']');
            return redirect()->to('/paises')->with('success', 'Registro criado com sucesso!');
        } else {
    ...
    {
      if (!$this->Carol->pode('PAISES.EDITAR')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
        ...
        if (!$registros) {
            return redirect()->to('/paises')->with('error', 'Registro não encontrado.');
        }
    ...
    public function update(int $id): string|ResponseInterface
    {
      if (!$this->Carol->pode('PAISES.EDITAR')) {
        ...
        if ($this->model->save($registros)) {
            $this->logAtividadesModel->addLog('Paises: registro atualizado [' . $id . '-' . $registro->nome . ']');
            return redirect()->to('/paises')->with('success', 'Registro atualizado com sucesso!');
        } else {
            return redirect()->back()->withInput()->with('errors', $this->model->errors());
        }
    }
    ...
    {
      if (!$this->Carol->pode('PAISES.EXCLUIR')) {
        return redirect()->to(site_url())->with('error', 'Acesso negado.');
      }
      ...
      $nome = $this->model->getCampo('nome', ['id' => $id]);
        ...
        if ($this->model->delete($id)) {
            $this->logAtividadesModel->addLog('Paises: registro excluído [' . $id . '-' . $nome . ']');
            return redirect()->to('/paises')->with('success', 'Registro excluído com sucesso!');
        } else {
